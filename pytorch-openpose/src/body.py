...
def to_heatmaps(heatmap_list, stride, imageToTest_padded, pad, oriImg):
    heatmap_list_converted = []
    for heatmap in heatmap_list:
        ...
        heatmap_list_converted.append(heatmap)

    return heatmap_list_converted
...
class Body(object):
    ...
    def __call__(self, oriImg):
        ...
        for m in range(len(multiplier)):
            ...
            with torch.no_grad():
                Mconv7_stage6_L1, Mconv7_stage6_L2, heatmap_list = self.model(data)
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        score_with_dist_prior = sum(score_midpts) / len(score_midpts) + min(
                            0.5 * oriImg.shape[0] / (norm+1e-5) - 1, 0)
